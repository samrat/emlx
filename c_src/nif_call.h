...
#define nif_call_evaluated                                                     \
  NIF_CALL_SYMBOL(NIF_CALL_NAMESPACE, nif_call_evaluated)
#define destruct_nif_call_res                                                  \
  NIF_CALL_SYMBOL(NIF_CALL_NAMESPACE, destruct_nif_call_res)
#endif
...
static ERL_NIF_TERM make_nif_call(ErlNifEnv *caller_env, ErlNifPid evaluator,
                                  ERL_NIF_TERM fun, ERL_NIF_TERM args);
static ERL_NIF_TERM nif_call_evaluated(ErlNifEnv *env, int argc,
...
CallbackNifRes *prepare_nif_call(ErlNifEnv *env) {
  CallbackNifRes *res = (CallbackNifRes *)enif_alloc_resource(
      CallbackNifRes::type, sizeof(CallbackNifRes));
  if (!res)
    return NULL;
...
static ERL_NIF_TERM make_nif_call(ErlNifEnv *caller_env, ErlNifPid evaluator,
  ...
  if (!callback_res)
    return CallbackNifRes::kAtomENOMEM;

  ERL_NIF_TERM callback_term =
      enif_make_resource(caller_env, (void *)callback_res);
  ERL_NIF_TERM data =
      enif_make_copy(callback_res->msg_env,
...
static int nif_call_onload(ErlNifEnv *env) {
  ...
  rt = enif_open_resource_type(env, "Elixir.NifCall.NIF", "CallbackNifRes",
                               destruct_nif_call_res, ERL_NIF_RT_CREATE, NULL);
  if (!rt)
    return -1;
